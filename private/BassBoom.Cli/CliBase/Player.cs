...
namespace BassBoom.Cli.CliBase
{
    internal static class Player
    {
        ...
        [
            ...
            new(LanguageTools.GetLocalized("BASSBOOM_APP_COMMON_KEYBINDING_VOLBOOST"), ConsoleKey.V, ConsoleModifiers.Shift),
            ...
            new(LanguageTools.GetLocalized("BASSBOOM_APP_COMMON_KEYBINDING_VISUALIZER"), ConsoleKey.V),
        ...
        public static void PlayerLoop()
        {
            ...
            screenPart.AddDynamicText(() =>
            {
                ...
                // Get the position
                int renderedPosition = BassBoomCli.basolia.IsOpened() ? BassBoomCli.basolia.GetCurrentDuration() : 0;
                var posSpan = BassBoomCli.basolia.IsOpened() ? BassBoomCli.basolia.GetCurrentDurationSpan() : new();
                ...
                string lyricTextStr = Common.CurrentCachedInfo.LyricInstance is not null && BassBoomCli.basolia.IsPlaying() ? $"{finalLyric}" : "";
            ...
            while (!Common.exiting)
            {
                try
                {
                    if (!playerScreen.CheckBufferedPart("BassBoom Player"))
                        playerScreen.AddBufferedPart("BassBoom Player", screenPart);
                    if (playerScreen.NeedsRefresh || playerScreen.RefreshWasDone || BassBoomCli.basolia.IsPlaying())
                        ScreenTools.Render();
                    ...
                    if (BassBoomCli.basolia.IsPlaying())
                        position = BassBoomCli.basolia.GetCurrentDuration();
            ...
            switch (keystroke.Key)
            {
                ...
                case ConsoleKey.C:
                    ...
                    else
                    {
                        if (BassBoomCli.basolia is null)
                            throw new BasoliaException(LanguageTools.GetLocalized("BASSBOOM_BASOLIA_EXCEPTION_BASOLIAMEDIA"), mpg123_errors.MPG123_BAD_HANDLE);
                        Common.CurrentCachedInfo.RepeatCheckpoint = BassBoomCli.basolia.GetCurrentDurationSpan();
                    }
                    break;
                case ConsoleKey.F2:
        ...
        private static void HandlePlay()
        {
            try
            {
                ...
                foreach (var musicFile in Common.cachedInfos.Skip(Common.currentPos - 1))
                {
                    ...
                    BassBoomCli.basolia.SeekToFrame(position);
                    Common.paused = false;
